...
class SaeTClientV2
{
   ...
   public function wbo_get_user_info($token){
        import("Org.Util.SaeTOAuthV2",dirname(dirname(dirname(__FILE__))));
        if($_SESSION['token']['access_token']){
        $c = new \Org\Util\SaeTClientV2( C('WB_AKEY') ,C('WB_SKEY'), $_SESSION['token']['access_token'] );
        $uid_get = $c->get_uid();
        $uid = $uid_get['uid'];
        ...
        $o = new \Org\Util\SaeTOAuthV2( C('WB_AKEY') , C('WB_SKEY') ); 
