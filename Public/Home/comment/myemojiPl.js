    ...
    $.fn.myEmoji = function (options) {
        var defaults = {
            ...
            postFunction: function () {
                //验证是否登录
                var is_login = verificationUserData();
                if(is_login==false){
                    return false;
                }
                //获取内容
                var content = InputText.html();
                //获取评论的主键id
                var id = $("input[name='star_id']").val();
                //获取评论的数据库名字
                var mysql = $("input[name='mysql']").val();
                //加密contenty
                ...
                pt['mysql']=mysql;
                if (content == '') {
                    layer.confirm('逗我玩儿那！！！', function (index) {
                        layer.closeAll('dialog');
                    });
                } else {
                    $.ajax({
                        url: "/Home/Talent/Index/User_reviews_articles_content",
                        type: "POST",
                        dataType:"json",
                        data:{content:htmlcontent,id:id,mysql:mysql},
